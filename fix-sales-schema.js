...
async function migrateSalesOrderItems() {
  ...
  try {
    ...
    try {
      ...
      console.log('2. If using a direct URL, make sure DIRECT_URL is also in your .env file');
      ...
      throw new Error('Database connection failed. Fix connection issues before proceeding.');
    }

    // Step 1: Get all existing sales order items
    const existingItems = await prisma.salesOrderItem.findMany();
    console.log(`Found ${existingItems.length} sales order items to migrate`);
    ...
    const hasThreadPurchaseId = columnsCheck.some(c => c.column_name === 'threadPurchaseId');
    const hasFabricProductionId = columnsCheck.some(c => c.column_name === 'fabricProductionId');

    console.log(`Column check: threadPurchaseId=${hasThreadPurchaseId}, fabricProductionId=${hasFabricProductionId}`);
    ...
    // Drop relevant constraints one by one
      ...
      if (constraint.column_name === 'productId' &&
          (constraint.foreign_table_name === 'ThreadPurchase' || constraint.foreign_table_name === 'FabricProduction')) {
    ...
    for (const item of existingItems) {
      try {
        if (item.productType === 'THREAD') {
          console.log(`Updating thread item ${item.id} with productId=${item.productId}`);
          await prisma.salesOrderItem.update({
            where: { id: item.id },
            data: { threadPurchaseId: item.productId }
          });
        } else if (item.productType === 'FABRIC') {
          console.log(`Updating fabric item ${item.id} with productId=${item.productId}`);
          await prisma.salesOrderItem.update({
            where: { id: item.id },
