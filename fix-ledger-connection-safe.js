        ...
        if (envContent.includes("LEDGER_DATABASE_URL")) {
            ...
            if (mainDbMatch && mainDbMatch[1]) {
                ...
                if (updatedContent !== envContent) {
                    ...
                    console.log(
                        "✅ Updated LEDGER_DATABASE_URL with schema parameter",
                    );
                    updated = true;
                }
            }
            ...
            if (mainDbMatch && mainDbMatch[1]) {
                ...
                console.log(
                    "✅ Added LEDGER_DATABASE_URL with schema parameter",
                );
                updated = true;
            }
        }
        ...
        if (envContent.includes("LEDGER_DIRECT_URL")) {
            ...
            if (mainDirectMatch && mainDirectMatch[1]) {
                ...
                if (updatedContent !== envContent) {
                    fs.writeFileSync(envPath, updatedContent);
                    console.log(
                        "✅ Updated LEDGER_DIRECT_URL with schema parameter",
                    );
                    updated = true;
                }
            }
...
function pushModifiedSchema() {
    ...
    try {
        ...
        execSync(
            "npx prisma generate --schema=prisma/schema-ledger-modified.prisma",
            {
                stdio: "inherit",
            },
        );
        ...
        console.log("\nPushing modified schema to database...");
        execSync(
            "npx prisma db push --schema=prisma/schema-ledger-modified.prisma",
            {
                stdio: "inherit",
            },
        );
...
async function main() {
    ...
    console.log("\n📋 Summary:");
    console.log(
        `- Connection Strings Update: ${connectionsUpdated ? "✅ Success" : "❌ Failed"}`,
    );
    console.log(`- Schema Push: ${schemaPushed ? "✅ Success" : "❌ Failed"}`);
    console.log(
        `- Migration Script Creation: ${migrationCreated ? "✅ Success" : "❌ Failed"}`,
    );

    console.log("\n📌 Next steps:");
    console.log("1. Run migration script: node migrate-inventory-data.js");
    console.log(
        "2. Generate the client with the original schema: npx prisma generate --schema=prisma/schema-ledger.prisma",
    );
