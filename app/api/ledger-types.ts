...
// NOTE: This is a duplicate of app/lib/ledger-types.ts
// It exists here to solve module resolution issues with API routes
// Any changes here should also be made to app/lib/ledger-types.ts
...
/**
 * Standard API response for ledger operations
 */
...
export class LedgerValidator {
     ...
     */
    static validateBill(bill: Partial<Bill>): {
        isValid: boolean;
        errors: string[];
    } {
        const errors: string[] = [];
        ...
        if (bill.amount === undefined || bill.amount < 0)
    ...
    static validateTransaction(transaction: Partial<Transaction>): {
        ...
        if (
            ...
            errors.push(
                "At least one of party, bill, or bank account must be specified",
            );
     ...
     * @returns Validation result with errors if any
     */
    static validateParty(party: Partial<Party>): {
        isValid: boolean;
        errors: string[];
    } {
        const errors: string[] = [];
