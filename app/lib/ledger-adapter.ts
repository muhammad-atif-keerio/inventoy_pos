...
export class LedgerClientAdapter implements LedgerDbClient {
    ...
    get khata() {
        return {
            ...
            findMany: async (args?: any) => {
                try {
                    const results =
                        await this.client.ledgerKhata.findMany(args);
            ...
            findUnique: async (args: any) => {
                try {
                    const result =
                        await this.client.ledgerKhata.findUnique(args);
            ...
            findFirst: async (args: any) => {
                try {
                    const result =
                        await this.client.ledgerKhata.findFirst(args);
                    return result as unknown as Khata | null;
                } catch (error) {
    ...
    // Transaction adapter
    get transaction() {
        return this.createModelProxy(
            this.client.ledgerTransaction,
            "transaction",
        );
    }
            ...
            switch (modelType) {
                ...
                case "bill":
                    itemData = {
                        ...
                        status:
                            i % 2 === 0 ? BillStatus.PENDING : BillStatus.PAID,
                    };
                    break;
                ...
                case "party":
                    itemData = {
                        name: `Mock Party ${i + 1}`,
                        type:
                            i % 2 === 0 ? PartyType.VENDOR : PartyType.CUSTOMER,
                        khataId: 1,
    ...
    async $connect(): Promise<void> {
        try {
            ...
            console.error(
                "LedgerClientAdapter: Failed to connect to database:",
                error,
            );
            console.log(
                "LedgerClientAdapter: Will use mock data in development mode",
            );
    ...
    async $disconnect(): Promise<void> {
        try {
            ...
            console.error(
                "LedgerClientAdapter: Error disconnecting from database:",
                error,
            );
            ...
            if (process.env.NODE_ENV === "production") {
                throw new LedgerError(
                    "Failed to disconnect from ledger database",
                    {
    ...
    async checkConnection(): Promise<boolean> {
        try {
            ...
            console.error(
                "LedgerClientAdapter: Connection check failed:",
                error,
            );
            return false;
        }
    ...
    async reconnect(retryCount = 0, maxRetries = 3): Promise<boolean> {
        if (retryCount >= maxRetries) {
            console.error(
                `LedgerClientAdapter: Failed to reconnect after ${maxRetries} attempts`,
            );
            return false;
        }
        ...
        try {
            ...
            console.log(
                `LedgerClientAdapter: Reconnection attempt ${retryCount + 1}/${maxRetries} in ${delay}ms`,
            );
            ...
            if (isConnected) {
                console.log(
                    "LedgerClientAdapter: Successfully reconnected to database",
                );
                return true;
            }
            ...
            console.error(
                `LedgerClientAdapter: Error during reconnection attempt ${retryCount + 1}:`,
                error,
            );
            return this.reconnect(retryCount + 1, maxRetries);
        }
