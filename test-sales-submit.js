...
import { PrismaClient, PaymentMode, PaymentStatus, ProductType } from "@prisma/client";
...
async function testSalesSubmission() {
  try {
    ...
    if (threadPurchases.length === 0 || fabricProductions.length === 0) {
      console.error("No thread purchases or fabric productions found. Cannot run test.");
      return;
    }
    ...
    if (result.salesOrder && result.salesOrder.items) {
      ...
      for (const item of items) {
        if (item.productType === "THREAD") {
          console.log(`Thread item: productId=${item.productId}, threadPurchaseId=${item.threadPurchaseId}, fabricProductionId=${item.fabricProductionId}`);
          
          // Verify threadPurchaseId is set correctly
          if (item.threadPurchaseId !== threadPurchase.id) {
            console.error(`❌ Thread item has incorrect threadPurchaseId. Expected ${threadPurchase.id}, got ${item.threadPurchaseId}`);
          } else {
            console.log(`✅ Thread item has correct threadPurchaseId: ${item.threadPurchaseId}`);
          }
        } else if (item.productType === "FABRIC") {
          console.log(`Fabric item: productId=${item.productId}, threadPurchaseId=${item.threadPurchaseId}, fabricProductionId=${item.fabricProductionId}`);
          
          // Verify fabricProductionId is set correctly
          if (item.fabricProductionId !== fabricProduction.id) {
            console.error(`❌ Fabric item has incorrect fabricProductionId. Expected ${fabricProduction.id}, got ${item.fabricProductionId}`);
          } else {
            console.log(`✅ Fabric item has correct fabricProductionId: ${item.fabricProductionId}`);
          }
        }
      }
    }
