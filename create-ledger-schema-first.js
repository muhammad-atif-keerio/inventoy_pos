...
function pushSchema() {
  ...
  try {
    ...
    console.log("\nPushing schema to database...");
    execSync("npx prisma db push --schema=prisma/schema-ledger-schema.prisma --accept-data-loss", {
      stdio: "inherit",
    });
...
async function main() {
  ...
  // Summary
  console.log("\n📋 Summary:");
  console.log(`- Schema Creation: ${schemaCreated ? "✅ Success" : "❌ Failed"}`);
  console.log(`- Schema Push: ${schemaPushed ? "✅ Success" : "❌ Failed"}`);
  console.log(`- Test Script: ${testScriptCreated ? "✅ Success" : "❌ Failed"}`);
  ...
  if (schemaPushed) {
    ...
    console.log("3. The application will fall back to mock data in development mode");
  }
}
