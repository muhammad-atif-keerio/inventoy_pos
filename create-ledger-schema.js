...
function updateEnvFile() {
    ...
    try {
        ...
        if (mainDbMatch && mainDbMatch[1]) {
            ...
            console.log(
                "✅ Updated .env file with clean URLs (no schema parameter)",
            );
            return true;
        } else {
...
function setupLedgerSchema() {
    ...
    try {
        ...
        execSync(
            "npx prisma db push --schema=prisma/schema-ledger-schema.prisma",
            {
                stdio: "inherit",
            },
        );
...
function createDefaultKhata() {
    ...
    try {
        const scriptPath = path.join(
            __dirname,
            "create-default-khata-prefixed.js",
        );
...
async function main() {
    ...
    console.log("\n📋 Summary:");
    console.log(
        `- Environment Update: ${envUpdated ? "✅ Success" : "❌ Failed"}`,
    );
    console.log(`- Schema Setup: ${schemaSetup ? "✅ Success" : "❌ Failed"}`);
    console.log(`- Code Update: ${codeUpdated ? "✅ Success" : "❌ Failed"}`);
    console.log(
        `- Default Khata Script: ${defaultKhataScript ? "✅ Success" : "❌ Failed"}`,
    );

    console.log("\n📌 Next steps:");
    console.log(
        "1. Run the default khata script: node create-default-khata-prefixed.js",
    );
