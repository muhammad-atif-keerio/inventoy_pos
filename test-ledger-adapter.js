...
async function runTest() {
  try {
    ...
    bills.forEach(b => console.log(`- ${b.id}: ${b.billNumber}, Amount: ${b.amount}, Status: ${b.status}`));
    
    // Test error handling by trying to find a non-existent item
    console.log('\nTesting error handling (finding non-existent khata):');
    try {
      const nonExistentKhata = await adapter.khata.findUnique({ where: { id: 9999 } });
      console.log('Result:', nonExistentKhata ? 'Found (mock data)' : 'Not found');
    } catch (error) {
